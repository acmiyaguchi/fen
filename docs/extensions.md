...
The first-party `skills` extension (`extensions/behaviors/companions/skills/`) discovers and exposes Agent Skills.
...
| command | effect |
| --- | --- |
| `/skills` | Open the skills picker/detail panel. |
| `/skills <name>` | Jump directly to one skill. |
| `/skills list` | Text list with scopes, visibility, and paths. |
| `/skills visible\|hidden\|builtin\|user\|project\|cli` | Filtered list views. |

The extension also exposes a `discovered-skills` introspection snapshot for `/extensions skills` and diagnostics.
