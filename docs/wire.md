...
`seq` is a positive integer that strictly increases per sender.
...
`fen.util.wire.normalize` produces them with transport bounds applied; truncated payloads carry `transport-truncated?`.
...
- `control-ack {ref, status, reason}` answers the control message whose `seq` is `ref`, with status `accepted`, `rejected`, or `applied`; `ref` is omitted only for a rejected line that had no readable `seq`.
