;; Low-level termbox2 drawing primitives, split from paint so input can depend on draw without cycling through paint.
...
(fn M.utf8-prefix-cols [s cols]
  ...
  (let [text (or s "")
    ...
    ;; ASCII byte length equals display columns, so string.sub clips; UTF-8-aware clipping only for non-ASCII overflow.
