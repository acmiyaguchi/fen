package = "fen-ext-todo"
version = "1-1"
rockspec_format = "3.0"

source = {
   url = "git+https://github.com/acmiyaguchi/fen.git",
   dir = "fen/extensions/behaviors/companions/todo",
}
...
if [ -n "${FEN_WORKSPACE:-}" ] && [ -f "$FEN_WORKSPACE/scripts/fennel-build.fnl" ]; then
  ...
  find . -type f -name '*.fnl' \
   ...
   install = {
      lua = {
         ["fen.extensions.todo"] = ".lrbuild/extensions/todo/init.lua",
         ["fen.extensions.todo.manifest"] = ".lrbuild/extensions/todo/manifest.lua",
         ["fen.extensions.todo.state"] = ".lrbuild/extensions/todo/state.lua",
      },
   },
}
