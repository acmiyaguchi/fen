...
;; @doc fen.extensions.builtin_commands.state.prompt.visible?
;; kind: data
;; signature: boolean
...
;; @doc fen.extensions.builtin_commands.state.prompt.cached-rows
;; kind: data
...
;; @doc fen.extensions.builtin_commands.state.prompt.cached-at
;; kind: data
;; signature: number
;; summary: Timestamp for the /prompt panel cache, used to avoid rebuilding prompt rows on every repaint.
;; tags: builtin commands state prompt cache

;; @doc fen.extensions.builtin_commands.state.prompt.cached-w
;; kind: data
;; signature: number
