...
(describe "core.skills.parse-frontmatter"
  (fn []
    ...
    (it "returns nil when name or description is missing"
      (fn []
        (let [path (.. tmp "/SKILL.md")]
          (write-file path "---\nname: solo\n---\n")
          (assert.is_nil (skills-mod.parse-frontmatter path)))))
  ...
  (fn []
    ...
    (before_each
      (fn []
        ...
        (tset package.loaded :core.skills nil)
        (set skills-mod (require :core.skills))
        ;; Stub project-skills-dir to point under tmp so cwd-relative scanning
        ;; doesn't pick up real files.
        (tset skills-mod :project-skills-dir
              (fn [] (.. tmp "/project-skills-empty")))))
    ...
    (it "merges --skills extra dirs (tagged scope=:cli)"
      (fn []
...
(describe "core.skills.system-prompt-section"
  (fn []
    ...
    (it "renders a bullet list with name, path, and description"
      (fn []
        (let [text (skills-mod.system-prompt-section
                     [{:name "greeter" :path "/x/SKILL.md"
                       :description "Greets" :scope :user}])]
          (assert.is_string text)
          (assert.is_truthy (string.find text "Available skills"))
          (assert.is_truthy (string.find text "greeter"))
          (assert.is_truthy (string.find text "/x/SKILL.md" 1 true))
          (assert.is_truthy (string.find text "Greets")))))))
